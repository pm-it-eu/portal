...
async function startEmailPolling() {
  ...
  if (initialConfigs.length === 0) {
    ...
    setInterval(async () => {
      try {
        ...
        if (configs.length > 0) {
          console.log(`📧 ${configs.length} E-Mail-Konfigurationen gefunden - starte Polling...`)
          await startPollingWithConfigs()
        }
      } catch (error) {
  ...
  // Starte Polling mit vorhandenen Konfigurationen
  await startPollingWithConfigs()
}

// Funktion für Polling mit Konfigurationen
async function startPollingWithConfigs() {
